...
type VideoJobInteractorImpl struct {
	videoJobRepo   repository2.VideoJobRepository
	videoRepo      repository2.VideoRepository
	finalVideoRepo repository2.FinalVideoRepository
}

func NewVideoJobInteractor(videoJobRepo repository2.VideoJobRepository, videoRepo repository2.VideoRepository, finalVideoRepo repository2.FinalVideoRepository) VideoJobInteractor {
...
func (vji *VideoJobInteractorImpl) TrimVideo(videoID int, trimStart string, trimEnd string) (int, error) {
	...
	job := domain2.NewVideoJob(videoID, domain2.Trim, map[string]interface{}{
...
func (vji *VideoJobInteractorImpl) ConcatVideos(videoIDs []int) (int, error) {
	...
	job := domain2.NewVideoJob(videoIDs[0], domain2.Concat, map[string]interface{}{
...
func (vji *VideoJobInteractorImpl) ExecuteJobs(jobIDs []int) ([]response.JobIDResponse, error) {
	...
	for _, job := range pendingJobs {
		job.UpdateStatus(domain2.InProgress)
